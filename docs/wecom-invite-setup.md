...
- 新客户添加甘老师时，企业微信回调带回新增客户事件及可能出现的一次性 `WelcomeCode`。个人名片方式通常没有网站渠道标识，但老师已确认所有新联系人都可发码。服务端验签、解密并核对企业 ID、成员 UserID 后生成 72 小时有效的 10 位邀请码，通过欢迎消息发送可点击的注册链接。相同客户的半客户/正式客户连续事件沿用第一个有效码。
- 学生或家长点开链接后邀请码自动填入。注册申请仍进入老师审核队列；家长绑定孩子必须由老师核对。旧的老师手动邀请码仍按指定身份和手机号使用。
- 验收应使用新的微信账号扫描个人名片二维码，核对免验证添加、20 秒内收到欢迎消息、注册链接预填、72 小时有效且一次使用后失效、后台出现待审核申请。还需确认只有新添加指定老师的客户会收到邀请码。
- 企业微信的欢迎消息只允许在刚添加后的短时间内发送；如果对方先开始聊天，企业微信可能拒绝发送。服务端只在接口确认成功时记录“已送达”；没收到邀请码的联系人仍可向老师索取手动邀请码。
