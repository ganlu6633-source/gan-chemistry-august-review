...
export function GuardianApp({ dashboard, session }: { dashboard: GuardianDashboardData; session: SessionIdentity }) {
  ...
  return <div className="guardian-dashboard">
    <section className="guardian-hero"><div><span className="eyebrow">30秒看懂本期学习</span><h1>{dashboard.student.displayName}的化学成长说明</h1><p>这里同时呈现进步与需要关注的地方，也会告诉您系统和甘老师已经做了什么。</p><p className="live-sync-note"><CheckCircle2 size={17} />本期已有 {dashboard.weeklyQuizCompleted} 轮即时小测同步到这里，完成后约10秒更新。</p></div><div className="completion-ring"><b>{dashboard.weeklyCompleted}/{dashboard.weeklyPlanned}</b><span>本期复习</span></div></section>
    <div className="guardian-columns">
      <section className="guardian-card"><div className="card-title positive-text"><CheckCircle2 /><div><span>本期明显进步</span><h2>已经获得什么</h2></div></div><ul className="plain-list">{dashboard.progress.length ? dashboard.progress.map((item) => <li key={item}><ChemText>{item}</ChemText></li>) : <li>本期还没有形成足够的稳定证据。</li>}</ul></section>
    ...
    <section className="guardian-next-action"><h2>老师下一步安排</h2><p>{dashboard.timeline.filter((event) => event.type === 'teacher_action').slice().sort((a, b) => b.at.localeCompare(a.at))[0]?.description || '暂未发布新的讲解安排；已有学习证据会供甘老师复核。'}</p></section>
    <section className="guardian-care-design" aria-labelledby="guardian-care-title"><div className="guardian-care-copy"><span className="eyebrow">甘老师为孩子设计的复习闭环</span><h2 id="guardian-care-title">当天定位，连续接稳</h2><p>每一步都从孩子真正学过的范围出发；系统负责整理证据，最终由甘老师判断怎样讲、推什么内容。</p></div><ol><li><Sparkles /><div><b>{dashboard.student.gradeBand === '初三' ? '12 道起步，最多 15 道' : '每天 1—8 道原题'}</b><span>{dashboard.student.gradeBand === '初三' ? '每题作答后按表现选择下一题，先把基础接稳。' : '一题对应一个细知识点，先找出真正需要回看的地方。'}</span></div></li><li><RotateCcw /><div><b>错题与不确定题优先</b><span>下一次换同知识点的不同原题；答对且确定后再提高难度。</span></div></li><li><BookOpenCheck /><div><b>仍不稳就人工复核</b><span>甘老师复核错因后安排讲解，观看反馈也会如实呈现。</span></div></li></ol></section>
    <section className="guardian-metrics">
      <article className="positive"><Sparkles /><b>{summary.full}</b><span>完全点亮</span><small>按现有证据汇总；详细记录可查看时间与题目</small></article>
