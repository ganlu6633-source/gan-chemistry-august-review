...
export function JuniorAdaptiveSession({
  ...
  async function submit() {
    if (!question || !payload.currentStepId || selected === null || feedback || busy) return
    setBusy(true)
    setError('')
    try {
  ...
  return <section className="learning-stage junior-adaptive-stage">
    ...
    <div className="stage-actions"><button className="secondary-button" disabled={busy} onClick={onExit}>稍后继续 / 返回计划</button>{feedback ? <button ref={primaryAction} className="primary-button" aria-keyshortcuts="Enter" disabled={busy || !pendingPayload} onClick={next}>{pendingPayload?.completed ? '完成今天学习' : '下一题'}<ChevronRight size={18} /></button> : <button ref={primaryAction} className="primary-button" aria-keyshortcuts="Enter" disabled={busy || selected === null} onClick={() => void submit()}>{busy ? '正在锁定第一次选择…' : '提交答案'}</button>}</div>
  </section>
}
