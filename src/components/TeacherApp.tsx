...
function StudentTable({ dashboard }: { dashboard: TeacherDashboardData }) { return <><div className="teacher-page-head"><div><span className="eyebrow">一个studentId贯穿全部系统</span><h1>学生档案</h1></div></div><div className="data-table"><div className="data-row head"><span>学生</span><span>年段</span><span>状态</span><span>初始化</span></div>{dashboard.students.map((student) => <div className="data-row" key={student.id}><span><b>{student.displayName}</b><small>{student.id.slice(0,8)}</small></span><span>{student.gradeBand}</span><span className={`status ${student.status}`}>{student.status}</span><span>{student.needsInitialDiagnostic ? '需要初始诊断' : '已有证据'}</span></div>)}</div></> }
