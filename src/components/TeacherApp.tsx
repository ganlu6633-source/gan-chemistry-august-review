...
function TeacherWorkspace({ onPreviewStudent }: { onPreviewStudent?: (studentId: string) => void }) {
  ...
  }).join('|') + planningAlerts.map((alert) => `${alert.id}:${alert.createdAt}:${alert.message}`).join('|')
...
export function ReviewPlanningAlerts({ alerts }: { alerts: NonNullable<TeacherDashboardData['planningAlerts']> }) {
  return <div className="source-pool-warning-list compact">{alerts.map((alert) => <article key={alert.id} className="blocking"><div><b>{alert.studentName}</b><span>{alert.planDate ? `${alert.planDate} 计划` : "后续计划"}</span></div><p><ChemText>{alert.message}</ChemText></p><small>{alert.kind === 'capacity' ? '原题容量核验未通过' : '个性化生成待重试'}</small></article>)}</div>
}
...
export function SourcePoolWarnings({ warnings, compact = false }: { warnings: NonNullable<TeacherDashboardData['sourcePoolWarnings']>; compact?: boolean }) {
  return <div className={compact ? 'source-pool-warning-list compact' : 'source-pool-warning-list'}>{warnings.map((warning) => {
    const details = (warning.conceptDetails || []).filter((detail) => warning.severity === 'progression'
      ? detail.difficultyLevels < 2
      : warning.severity === 'blocking'
      ? true
      : detail.missingQuestions > 0)
    return <article key={warning.id} className={warning.severity}><div><b><ChemText>{`${warning.gradeBand} · ${warning.skillTitle}`}</ChemText></b><span>{warning.severity === 'blocking' ? '今日题组会阻断' : warning.severity === 'progression' ? '答对后无法升级' : '跨日不重复容量不足'}</span></div><p><ChemText>{warning.message}</ChemText></p>{details.length > 0 && <ul className="source-pool-concept-details">{details.map((detail) => <li key={detail.conceptKey}><b><ChemText>{detail.conceptTitle}</ChemText></b><span>现有 {detail.availableQuestions} 题 · 需要 {detail.requiredQuestions} 题 · 还差 {detail.missingQuestions} 题 · {detail.difficultyLevels} 个难度层级</span></li>)}</ul>}<small>{warning.plannedStudentCount}名学生 · {warning.plannedDateCount}个计划日 · 题库已覆盖{warning.conceptCount}/{warning.expectedConceptCount}个权威细知识点</small></article>
  })}</div>
}
...
function PlanEditor({ dashboard }: { dashboard: TeacherDashboardData }) {
  ...
  return <><div className="teacher-page-head"><div><span className="eyebrow">课程目录与教学管理</span><h1>课程节点审核</h1></div></div><section className="teacher-panel"><p>这里用于维护课程目录与教师审核记录，不直接修改学生日期计划，也不单独控制初三正式题下发。</p><div className="editor-summary"><div><b>{dashboard.pendingCourseNodes}</b><span>课程节点待审核</span></div><div><b>3</b><span>调度模式</span></div><div><b>1—8</b><span>高中每日原题</span></div><div><b>12—15</b><span>初中自适应原题</span></div></div>{error && <div className="inline-alert" role="alert">{error}</div>}{loading ? <div className="center-loading"><RefreshCw className="spin" />正在读取课程节点……</div> : <div className="audit-list">{nodes.map((node) => <article key={node.id}><div><b><ChemText>{node.title}</ChemText></b><p>{node.grade_band} · {node.textbook_version} · {node.chapter}</p></div><button className="secondary-button" disabled={Boolean(busy)} onClick={() => void toggle(node)}>{busy === node.id ? '正在保存…' : node.teacher_approved ? '撤回批准' : '批准使用'}</button></article>)}</div>}</section></>
}
        ...
        {questions.map((question) => {
          ...
          return <details className="question-audit-card" key={question.id}>
            ...
            <div className="question-audit-detail">
              ...
              <section className="question-audit-analysis"><h3>{licensedHighSchoolSource ? '文字解析（原解析图为最终依据）' : nativeJuniorSource ? '文字解析（整套发布核验后下发）' : '审核文字解析'}</h3><div className="answer-explanation">{explanationParagraphs.map((item, paragraphIndex) => <p className={item.option ? undefined : 'is-unlabeled'} key={`${item.option ?? 'paragraph'}-${paragraphIndex}`}>{item.option && <b className="answer-option-label">{item.option}</b>}<span className="answer-explanation-text"><ChemText>{item.text}</ChemText></span></p>)}</div>{question.scaffold && <small>提示：<ChemText>{question.scaffold}</ChemText></small>}</section>
