...
function StudentManagement({ catalog, loading, onReload, onPreview }: { catalog: TeachingCatalog; loading: boolean; onReload: () => Promise<void>; onPreview?: (studentId: string) => void }) {
  ...
  return <>
    ...
    <p className="tm-help">不同学校请分别建班并安排各自进度；学生仍可自主选择题库中的已审核内容。新学生可以按参照同学的教材和已学内容设置起点，成绩与错题由本人重新积累。</p>
...
function ClassManagement({ catalog, onReload }: { catalog: TeachingCatalog; onReload: () => Promise<void> }) {
  ...
  return <section className="teacher-panel"><h2>学校与班级进度</h2><p className="tm-help">同一年段的不同学校分别建班，例如“一中·高一（1）班”和“二中·高一（1）班”；在课程安排中分别选择班级，可设置不同的学习进度。</p><form className="tm-form" onSubmit={save}><div className="tm-form-grid"><label>学校与班级名称<input value={name} maxLength={60} onChange={(event) => setName(event.target.value)} required placeholder="例如：一中·高一（1）班" /></label><label>班级年段<select value={grade} onChange={(event) => setGrade(event.target.value as GradeBand)}>{managementGrades.map((item) => <option key={item}>{item}</option>)}</select></label></div>{error && <div className="inline-alert" role="alert">{error}</div>}{message && <div className="success-message" role="status">{message}</div>}<div className="tm-actions"><button className="secondary-button" disabled={busy}>{busy ? '正在保存…' : editing ? '保存班级' : '创建班级'}</button>{editing && <button type="button" className="text-button" onClick={() => { setEditing(null); setName('') }}>取消编辑</button>}</div></form><div className="tm-class-list">{catalog.classes.map((item) => <article key={item.id}><div><b>{item.name}</b><small>{item.gradeBand} · {item.memberCount} 名学生</small></div><button className="table-action" disabled={busy} onClick={() => { setEditing(item); setName(item.name); setGrade(item.gradeBand); setError(''); setMessage('') }}>编辑班级</button></article>)}</div></section>
}
