...
describe('mixed REVIEW fine-concept targets', () => {
  ...
  it('uses the same exact target filter when issuing and submitting', () => {
    ...
    expect(accessSource).toContain('.select("id,student_id,plan_date,mode,skill_ids,target_concept_keys,question_count,round_limit,max_question_level,delivery_mode,self_study_release_id,teaching_managed,teaching_source_grade")')
