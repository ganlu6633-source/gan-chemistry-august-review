...
describe('high-school source-backed REVIEW backend contract', () => {
  ...
  it('opens every high-school demo day with the verified source release while keeping all demo answers read-only', () => {
    ...
    expect(accessFunction).toContain('if (!assetTargetIsDemo)')
    expect(accessFunction).toContain('question.render_mode === "image_primary"')
    expect(accessFunction).toContain('String(question.source_release_id || "") === activeAssetReleaseId')
    expect(accessFunction).toContain('.eq("source_release_id", activeFeedbackReleaseId)')
