-- A missing participation flag means an archived/unselected legacy profile,
-- not permission to resume the old misspelled student's curriculum.
-- Keep all historical rows; only current roster display/reference/target scope changes.
 ...
 if p_action='teaching_catalog' then
   return jsonb_build_object('catalogVersion',coalesce((select max(updated_at)::text from app_private.chem_teaching_topics),''),
    'topics',coalesce((with recursive tree(ancestor,descendant) as (
      ...
      mr as (select tr.ancestor,sum(m.candidate_count) qty from (select distinct tr.ancestor,m.id from tree tr join app_private.chem_teaching_materials m on tr.descendant=any(m.topic_ids) where m.status='needs_review') tr join app_private.chem_teaching_materials m on m.id=tr.id group by tr.ancestor)
