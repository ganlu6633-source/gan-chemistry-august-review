-- Reference changes keep target historical assignments. Material selection uses remaining independent source capacity.
-- No existing students, plans, questions, assets or answer records are rewritten by this migration.
begin;
 ...
 if p_action='teaching_catalog' then
   ...
   if v_op in ('create','update') then
     ...
     if v_ref.id is not null then
       ...
       -- A new reference replaces the future only. Keep the target's original
       -- historical date -> question IDs; never copy the donor's past answers or assignments.
       select coalesce(jsonb_object_agg(key,value),'{}') into v_assign
         from jsonb_each(coalesce(v_program->'questionAssignments','{}')) where key<v_today::text;
       select v_assign||coalesce(jsonb_object_agg(key,value),'{}') into v_assign
         from jsonb_each(coalesce(v_ref.metadata#>'{reviewProgram,questionAssignments}','{}')) where key>=v_today::text;
       -- Retaining an old fixed assignment map must not disable newly copied
       -- dynamic days. Store their actual dates explicitly, without rewriting plan rows.
       select coalesce(jsonb_agg(d order by d),'[]') into v_dynamic from (
         select distinct d from (
           select value d from jsonb_array_elements_text(coalesce(v_program->'dynamicAssignmentDates','[]')) where value<v_today::text
           union all
           select value d from jsonb_array_elements_text(coalesce(v_ref.metadata#>'{reviewProgram,dynamicAssignmentDates}','[]')) where value>=v_today::text
           union all
           select p.plan_date::text from public.chem_learning_plans p
             where p.student_id=v_ref.id and p.plan_date>=v_today and p.mode='REVIEW'
              and p.delivery_mode='legacy_round' and not p.teaching_managed
              and not(coalesce(v_ref.metadata#>'{reviewProgram,questionAssignments}','{}') ? p.plan_date::text)
         ) dates
       ) distinct_dates;
       ...
       v_program=v_program||jsonb_build_object('dynamicAssignmentDates',v_dynamic);
       if not (v_ref.metadata->'reviewProgram') ? 'questionAssignments' and v_assign='{}'::jsonb then v_program=v_program-'questionAssignments'; end if;
       ...
       update public.chem_students_v2 set metadata=jsonb_set(metadata,'{reviewProgram}',v_program)||jsonb_build_object('teacherSchedulingManaged',coalesce((v_ref.metadata->>'teacherSchedulingManaged')::boolean,false) or jsonb_array_length(v_dynamic)>0,'startingProgressReference',jsonb_build_object('studentId',v_ref.id,'copiedAt',now()),'confirmedLearnedSkillIds',coalesce(v_ref.metadata->'confirmedLearnedSkillIds','[]')),teaching_plan_items=v_ref.teaching_plan_items,updated_at=now() where id=v_id;
     end if;
     if v_op='create' then
...
CREATE OR REPLACE FUNCTION app_private.chem_build_teaching_preview(p_data jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
declare v_ids uuid[]; v_item jsonb; v_rows jsonb='[]'; v_warnings jsonb='[]'; v_student record; v_topic record; v_q record;
 ...
 v_source record; v_candidate_ids text[]; v_candidate_used text[]; v_best_used text[]; v_best_count integer;
begin
 if p_data->>'targetType'='student' then
   select array_agg(id order by id) into v_ids from public.chem_students_v2 where id=(p_data->>'targetId')::uuid and record_status='active' and coalesce(metadata#>>'{reviewProgram,participating}','false')='true' and not coalesce((metadata->>'demo')::boolean,false);
 elsif p_data->>'targetType'='class' then
   select array_agg(id order by id) into v_ids from public.chem_students_v2 where teaching_class_id=(p_data->>'targetId')::uuid and record_status='active' and coalesce(metadata#>>'{reviewProgram,participating}','false')='true' and not coalesce((metadata->>'demo')::boolean,false);
 ...
 for v_student in select * from public.chem_students_v2 where id=any(v_ids) order by id loop
   ...
   for v_item in select value from jsonb_array_elements(p_data->'items') with ordinality a(value,n) order by value->>'date',n loop
     ...
     if exists(select 1 from public.chem_learning_plans p where p.student_id=v_student.id and p.mode='REVIEW' and p.plan_date=v_date and (p.id=any(v_keep) or app_private.chem_teaching_plan_started(p.id))) then
       ...
       -- Rank source releases by the questions this student can actually receive.
       -- Dry-run the same identity checks used by the final selection, including
       -- past answers/locks, retained assignments and earlier items in this preview.
       v_best_count=-1; v_best_used=v_all_used;
       for v_source in select q.grade_band,q.source_release_id
         ...
         group by q.grade_band,q.source_release_id
         order by (q.grade_band=v_student.grade_band) desc,count(*) desc,q.grade_band,q.source_release_id loop
         v_candidate_ids='{}'; v_candidate_used=v_all_used;
         for v_q in select q.* from app_private.chem_teaching_ready_questions q
           join app_private.chem_teaching_question_topics m on m.question_id=q.id
           where m.topic_id=v_topic.id and q.grade_band=v_source.grade_band and q.source_release_id=v_source.source_release_id
           order by q.level,q.id loop
           ...
           if v_candidate_used && v_used then continue; end if;
           v_candidate_ids=array_append(v_candidate_ids,v_q.id); v_candidate_used=v_candidate_used||v_used;
           exit when cardinality(v_candidate_ids)=v_count;
         end loop;
         if cardinality(v_candidate_ids)>v_best_count then
           v_best_count=cardinality(v_candidate_ids); v_selected=v_candidate_ids; v_best_used=v_candidate_used;
           v_grade=v_source.grade_band; v_release=v_source.source_release_id;
         end if;
         exit when v_best_count=v_count;
       end loop;
       v_all_used=v_best_used;
...
revoke all on function public.chem_teacher_management(text,jsonb,text,text),app_private.chem_build_teaching_preview(jsonb) from public,anon,authenticated;
