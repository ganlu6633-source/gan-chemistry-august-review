...
function ownedPlanDeliveryContext(plan: Record<string, unknown>, studentGrade: string) {
  try {
    return teachingPlanContext(plan, studentGrade);
  } catch (error) {
...
async function teachingReadyQuestions(grade: string, questionIds: string[]) {
  if (!questionIds.length) return [] as Array<{ question_id: string; source_release_id: string }>;
  const ids = [...new Set(questionIds)];
  if (ids.length > 256) throw new RequestError(422, "本次课程候选题数量超限。");
  const result = await supabase.rpc("chem_teaching_ready_question_ids", { p_grade: grade, p_question_ids: ids });
  if (result.error) throw result.error;
  return (result.data || []) as Array<{ question_id: string; source_release_id: string }>;
}
...
const questionShape = (row: Record<string, unknown>, secureLicensedHighSchoolReview = false, managedChoice = false) => {
  const hideSolution = managedChoice || shouldHideLicensedHighSchoolSolution(row, secureLicensedHighSchoolReview);
  return {
    ...
    secureFeedbackRequired: hideSolution,
...
async function studentLearningRecord(studentId: string) {
  ...
  const [skillsResult, statesResult, plansResult, attemptsResult, cardsResult] = await Promise.all([
    supabase.from("chem_skills").select("id,title,module_id,max_level,grade_band").eq("active", true).order("module_id"),
  ...
  const skills = (skillsResult.data || []).filter((skill) => skill.grade_band === gradeBand || managedRecordSkills.has(String(skill.id)) || answers.some((answer) => answer.skill_id === skill.id));
...
async function juniorSessionPayload(studentId: string, planId: string): Promise<Record<string, unknown>> {
  ...
  if (session.status === "completed") return { deliveryMode: "junior_adaptive", plan: studentPlan, cards: studentCards, session: sessionSummary(), currentQuestion: null, completed: true, optionPractice };
  if (session.status !== "active") {
  ...
  const issued = await supabase.rpc("chem_junior_issue_option_step", {
    p_session_id: String(session.id),
    p_student_id: studentId,
    p_branch_id: branch?.branchId ?? null,
...
async function futurePlanPreviewPayload(studentId: string, planId: string): Promise<Record<string, unknown>> {
  ...
  const managedPreview = ownedPlanDeliveryContext(plan, String(profile.grade_band)).managed;
  if (String(profile.grade_band) === "初三" && !managedPreview) {
...
async function studentDashboard(studentId: string) {
  ...
  const skillResult = await supabase.from("chem_skills")
    ...
    .eq("active", true)
    .or(`grade_band.eq.${String(profileResult.data.grade_band)},id.in.(${[...new Set(plans.filter((plan) => plan.teaching_managed === true).flatMap((plan) => Array.isArray(plan.skill_ids) ? plan.skill_ids.map(String) : []))].join(",") || "__none__"})`)
    .order("module_id");
...
async function openSelfStudy(studentId: string, skillId: string, conceptKey: string, releaseId: string, readOnlyPreview = false, options: StartPlanOptions = {}) {
  if (!validUuid(studentId) || !validUuid(releaseId)
    || !/^[A-Za-z0-9_]{1,80}$/.test(skillId) || !/^[A-Za-z0-9_]{1,100}$/.test(conceptKey)) {
    throw new RequestError(400, "请选择题库中的学生和知识点。");
  }
  const profile = await supabase.from("chem_students_v2").select("grade_band,record_status,metadata").eq("id", studentId).single();
  if (profile.error) throw profile.error;
  const grade = String(profile.data.grade_band);
  ...
  if (existing.data?.length) {
    ...
    if (!attempts.data?.length) return startPlanPayload(studentId, String(existing.data[0].id), {
      studentOpen: true, includeAnswerLocks: true, ...options,
    });
  }
  ...
  if (readOnlyPreview) {
    const digest = await sha256(`teacher-self-study:${studentId}:${skillId}:${conceptKey}:${releaseId}:${(existing.data || []).length + 1}`);
    const id = `${digest.slice(0, 8)}-${digest.slice(8, 12)}-4${digest.slice(13, 16)}-8${digest.slice(17, 20)}-${digest.slice(20, 32)}`;
    return startPlanPayload(studentId, id, { studentOpen: true, planOverride: { id, ...planFields }, ...options });
  }
...
async function startPlanPayload(studentId: string, planId: string, options: StartPlanOptions = {}) {
  ...
  if (options.studentOpen && !demoProfile && options.previewRound !== undefined && !options.teacherSimulation) {
  ...
  if (formalHighSchoolReview && reviewProfile.gradeBand === "高一" && !delivery.managed) {
  ...
  const assignedQuestionIds = formalHighSchoolReview || delivery.managed
    ? programQuestionIds(gradeResult.data.metadata, String(plan.plan_date)) : null;
  if (!teachingAssignmentValid(delivery.managed, assignedQuestionIds, questionCount)) {
    throw new RequestError(422, "老师安排的固定题目不完整，请联系甘老师重新排课。");
  }
  if (delivery.managed) {
    const ready = await teachingReadyQuestions(delivery.sourceGrade, assignedQuestionIds!);
    if (ready.length !== assignedQuestionIds!.length || new Set(ready.map((row) => row.source_release_id)).size !== 1) {
      throw new RequestError(422, "老师安排的题目来源尚未就绪，请联系甘老师核对。");
    }
  ...
  if (formalHighSchoolReview && !assignedQuestionIds && roundLimit === 1 && targetConceptKeys.length !== questionCount) {
  ...
  let eligibleQuestions = supabase
    .from("chem_questions")
    .select("*")
    .eq("grade_band", delivery.sourceGrade)
    .in("skill_id", skillIds)
    .eq("review_status", "approved")
    .eq("scope_status", "IN")
    .not("mother_id", "is", null);
  if (plan.mode === "REVIEW" && !delivery.managed) eligibleQuestions = eligibleQuestions.not("concept_key", "is", null);
  if (plan.mode === "REVIEW" && targetConceptKeys.length && !delivery.managed) {
  ...
  if (sourceControlledReview) {
    // Every source-controlled REVIEW surface, including the read-only demo, uses the
  ...
  if (plan.mode === "REVIEW") {
    ...
    const highSchoolReview = delivery.requiresImages && !delivery.managed;
  ...
  let historyRows: SourceAdaptiveHistory[] = historyData.map((answer) => {
    const snapshot = validQuestionSnapshot(answer.question_snapshot) ? answer.question_snapshot : null;
    ...
    return {
      ...
      parent_source_item_key: String(snapshot?.parentSourceItemKey || "") || null,
  ...
  if (delivery.managed || (plan.delivery_mode === "self_study" && delivery.sourceGrade === "初三")) {
    ...
    historyRows = [...historyRows, ...((juniorHistory.data || []) as Array<Record<string, unknown>>).map((row) => ({
      attempt_id: "legacy-junior-issued", question_id: String(row.question_id), mother_id: String(row.mother_id || "") || null,
      skill_id: "", concept_key: null, question_level: null, attempt_sequence: null, correct: false, uncertain: true,
      source_item_key: String(row.source_item_key || "") || null, parent_source_item_key: String(row.parent_source_item_key || "") || null,
      content_fingerprint: String(row.content_fingerprint || "") || null,
    }))];
  }
  ...
  if (plan.mode === "REVIEW" && !demoProfile && !delivery.managed && ["高一", "高二", "高三"].includes(String(gradeResult.data.grade_band))) {
  ...
  // Only explicitly reviewed per-option bindings may add practice. All four
  // identities, source revision and the active verified release are checked.
  const bindingResult = sourceControlledReview
    ? await supabase.rpc("chem_option_practice_bindings", { p_question_ids: adaptiveQuestions.map((question) => question.id) })
    : { data: [], error: null };
  if (bindingResult.error) throw bindingResult.error;
  const bindingById = new Map(((bindingResult.data || []) as Array<Record<string, unknown>>)
    .map((row) => [String(row.question_id), row.configuration]));
  const baseQuestions = adaptiveQuestions.map((question) => ({ ...question,
    source_info: { ...(question.source_info as Record<string, unknown> | null),
      optionPractice: bindingById.get(question.id) } }));
  const reserveIds = [...new Set(baseQuestions.flatMap((question) =>
    optionPracticeBindings(question).flatMap((binding) => binding.candidates.map((item) => item.questionId))))];
  const reserveResult = sourceControlledReview && reserveIds.length
    ? await supabase.from("chem_questions").select("*").in("id", reserveIds)
      .eq("grade_band", delivery.sourceGrade).eq("source_kind", delivery.sourceKind)
      .eq("review_status", "approved").eq("scope_status", "IN").eq("usable_for_review", true)
      .eq("render_mode", delivery.renderMode).eq("source_release_id", activeSourceReleaseId!)
    : { data: [], error: null };
  if (reserveResult.error) throw reserveResult.error;
  const futureAssignments = (gradeResult.data.metadata?.reviewProgram?.questionAssignments ?? {}) as Record<string, unknown>;
  const reservedBaseIds = new Set(Object.values(futureAssignments).flatMap((ids) => Array.isArray(ids) ? ids.map(String) : []));
  const allowedReserveSkills = delivery.managed ? null : programReviewSkillIds(gradeResult.data.metadata)
    ?? (gradeResult.data.grade_band === "高一" ? confirmedHighOneSkillIds(gradeResult.data.metadata) : null);
  const managedReserveReadyIds = delivery.managed && reserveIds.length
    ? new Set((await teachingReadyQuestions(delivery.sourceGrade, reserveIds)).filter((row) => String(row.source_release_id) === activeSourceReleaseId).map((row) => String(row.question_id))) : null;
  const reserves = await excludeHeldQuestions(((reserveResult.data || []) as SourceAdaptiveQuestion[]).filter((question) =>
    !reservedBaseIds.has(question.id) && Boolean(question.mother_id) && (delivery.managed || Boolean(question.concept_key))
    && (!managedReserveReadyIds || managedReserveReadyIds.has(question.id))
    && (allowedReserveSkills === null || allowedReserveSkills.includes(String(question.skill_id)))
    && (maxQuestionLevel === null || Number(question.level) <= maxQuestionLevel)
    && Number.isInteger(question.correct_option) && Number(question.correct_option) >= 0 && Number(question.correct_option) <= 3
    && String(question.stem || "").trim() && String(question.explanation || "").trim()
    && Array.isArray(question.options) && question.options.length === 4
    && question.options.every((option) => typeof option === "string" && option.trim())
    && (!delivery.requiresImages || hasRequiredReviewSourceAssets(question.asset_refs))));
  const allPossible = [...baseQuestions, ...reserves];
  const rawLocks: Array<Record<string, unknown>> = effectiveOptions.previewAnswers
    ? effectiveOptions.previewAnswers.map((answer) => {
      const question = allPossible.find((item) => item.id === answer.questionId);
      if (!question || answer.revisionToken !== question.question_revision_token
...
async function answerLocks(studentId: string, planId: string, sequence: number) {
  // Read the complete round, including locks whose question has since left a
  // binding or source release. Such changes must fail closed, not erase work.
  const result = await supabase.rpc("chem_current_plan_answer_locks", {
    p_student_id: studentId, p_plan_day_id: planId, p_attempt_sequence: sequence,
  });
  if (result.error) throw result.error;
  return (result.data || []) as Array<Record<string, unknown>>;
}

type JuniorPreviewAnswer = { stepId: string; selectedOption: number; revisionToken?: string | null; uncertain?: boolean; durationSec?: number };
...
async function juniorPreviewPayload(studentId: string, planId: string, previewAnswers: JuniorPreviewAnswer[] = []) {
  if (!validUuid(studentId) || !validUuid(planId) || previewAnswers.length > 15
    ...
    throw new RequestError(400, "初三模拟答题信息无效。");
  }
  const [planResult, profileResult] = await Promise.all([
    supabase.from("chem_learning_plans").select("*").eq("id", planId).eq("student_id", studentId).maybeSingle(),
    supabase.from("chem_students_v2").select("grade_band,textbook_version,metadata,record_status").eq("id", studentId).single(),
  ...
  if (!plan || !isJuniorAdaptivePlan(plan) || profile.grade_band !== "初三"
    || profile.record_status !== "active" || profile.textbook_version !== JUNIOR_TEXTBOOK_VERSION) {
  ...
  if (curriculumResult.error) throw curriculumResult.error;
  const curriculum = curriculumResult.data as Record<string, unknown> | null;
  if (!curriculum || !programAllowsJuniorUnit(profile.metadata, curriculum.unit_id)
    || !futurePreviewInstructionalTextIsSafe([curriculum.title, curriculum.knowledge_summaries])) {
    throw new RequestError(422, "这节课尚未完成审核发布。");
  }
  const skillIds = Array.isArray(curriculum.knowledge_skill_ids) ? curriculum.knowledge_skill_ids.map(String) : [];
  ...
  const cardOrder = new Map(skillIds.map((skillId, index) => [skillId, index]));
  const studentCards = [...cards].sort((a, b) => (cardOrder.get(String(a.skill_id)) ?? 99) - (cardOrder.get(String(b.skill_id)) ?? 99))
    .map(studentProvenanceFreeCardShape);
  if (studentCards.some((card) => !studentInstructionalCardTextIsSafe(card))) throw new RequestError(422, "知识卡内容尚未完成显示审核。");
  ...
  let stepCount = realSteps.length;
  let correctCount = realSteps.filter((step) => step.correct === true).length;
  let answeredCount = realSteps.filter((step) => step.answered_at).length;
  ...
  for (const answer of previewAnswers) {
    if (!currentQuestion || !currentStepId || answer.stepId !== currentStepId
      ...
      throw new RequestError(409, "模拟答题顺序或原题版本已变化，请重新打开当天学习。");
    }
    ...
    if (historyIndex >= 0) history[historyIndex] = virtual;
    else history.push(virtual);
    const activeBranch = optionState.branches.find((branch) => branch.status === "practicing"
    ...
    if (activeBranch) {
      ...
      activeBranch.total = target;
      if (activeBranch.answered >= target) {
        activeBranch.status = branchHistory.slice(-3).length === 3 && branchHistory.slice(-3).every(Boolean)
          ? "consolidated" : "needs_practice";
  ...
  try {
    ...
    if (body.action === "question_asset") {
      ...
      if (identity.role !== "teacher") {
        ...
        const profile = await supabase
          ...
          .maybeSingle();
        if (profile.error) throw profile.error;
        if (!profile.data || !["初三", "高一", "高二", "高三"].includes(String(profile.data.grade_band)) || profile.data.record_status !== "active") {
    ...
    if (body.action === "question_feedback") {
      ...
      const submittedRevisionToken = body.data?.revisionToken === null || body.data?.revisionToken === undefined
        ...
        : String(body.data.revisionToken);
      const previewRound = body.data?.previewRound === undefined ? undefined : Number(body.data.previewRound);
      const previewAnswers = identity.role === "teacher" && Array.isArray(body.data?.previewAnswers)
        ? body.data.previewAnswers as NonNullable<StartPlanOptions["previewAnswers"]> : undefined;
      if (previewAnswers && (previewAnswers.length > 48 || previewAnswers.some((answer) => !answer || typeof answer.questionId !== "string")
        || new Set(previewAnswers.map((answer) => answer.questionId)).size !== previewAnswers.length)) {
        return reply(req, { error: "预览答题记录格式无效。" }, 400);
      }
      if (
        !validUuid(planId)
        || !/^[a-zA-Z0-9_-]{1,160}$/.test(questionId)
      ...
      const payload = previewSelfStudy
        ? await openSelfStudy(targetId, String(previewSelfStudy.skillId || ""), String(previewSelfStudy.conceptKey || ""),
          String(previewSelfStudy.releaseId || ""), true, feedbackOptions)
        : await startPlanPayload(targetId, planId, feedbackOptions);
      if (String(payload.plan.id) !== planId) return reply(req, { error: "当前自主练习题组已变化，请重新打开。" }, 409);
      ...
      if (!readOnlyPreview) {
        ...
        const alreadyLocked = locks.some((item) => item.questionId === questionId);
        const nextQuestion = payload.questions.find((item) => !locks.some((lock) => lock.questionId === item.id));
        if (!alreadyLocked && nextQuestion?.id !== questionId) {
          return reply(req, { error: "请按当前题目顺序作答，先完成正在进行的原题或补练题。" }, 409);
        }
      }
    ...
    if (body.action === "preview_junior_open_session" && identity.role === "teacher") {
      const studentId = String(body.data?.studentId || "");
      const planId = String(body.data?.planId || "");
      const result = await juniorPreviewPayload(studentId, planId);
      return reply(req, { payload: result.payload });
    }

    if (body.action === "preview_junior_submit_step" && identity.role === "teacher") {
      const studentId = String(body.data?.studentId || "");
      const planId = String(body.data?.planId || "");
    ...
    if (body.action === "future_plan_preview" && ((identity.role === "student" && identity.studentId) || identity.role === "teacher")) {
      const planId = String(body.data?.planId || "");
      if (!validUuid(planId)) return reply(req, { error: "预习计划信息无效。" }, 400);
      const targetId = identity.role === "teacher" ? String(body.data?.studentId || "") : identity.studentId!;
      if (!validUuid(targetId)) return reply(req, { error: "预习学生信息无效。" }, 400);
      return reply(req, { preview: await futurePlanPreviewPayload(targetId, planId) });
    }
    ...
    if (body.action === "junior_submit_step" && identity.role === "student" && identity.studentId) {
      ...
      if (!validUuid(planId) || !validUuid(stepId) || !Number.isInteger(selectedOption) || selectedOption < 0 || selectedOption > 3
    ...
    if (body.action === "preview_self_study" && identity.role === "teacher") {
      ...
      return reply(req, { payload: await openSelfStudy(targetId, skillId, conceptKey, releaseId, true, { teacherSimulation: true }) });
    }
    ...
    if (body.action === "preview_start_plan" && identity.role === "teacher") {
      ...
      return reply(req, { payload: await startPlanPayload(targetId, planId, {
        studentOpen: true, teacherSimulation: true, includeAnswerLocks: true, previewRound,
      }) });
    }
    ...
    if (body.action === "submit_attempt" && identity.role === "student" && identity.studentId) {
      ...
      const formalHighSchoolReview = isFormalHighSchoolReview(formalReviewContext(plan, reviewProfile)) || delivery.managed;
      const managedAssignedIds = delivery.managed ? programQuestionIds(targetProfile.data.metadata, String(plan.plan_date)) : null;
      ...
      if (expandedQuestionCount > 48 || submittedAnswers.length !== expandedQuestionCount) {
        return reply(req, { error: `请完整提交服务器下发的 ${expandedQuestionCount} 道原题和补练题，再完成本轮。` }, 400);
      }
      ...
      if (expectedIds && (expectedIds.length !== questionIds.length || questionIds.some((id, index) => expectedIds[index] !== id))) {
        return reply(req, { error: "提交顺序或题目与服务器安排的原题及补练不一致。" }, 409);
      }
      if (assignedIds && (assignedIds.length !== questionCount || assignedIds.some((id) => !questionIds.includes(id)))) {
      ...
      if (
        ...
        && reviewProfile.gradeBand === "高一" && !delivery.managed
      ) {
      ...
      let questionQuery = supabase
        .from("chem_questions")
        .select("id,mother_id,skill_id,concept_key,level,grade_band,stem,options,correct_option,explanation,image_url,review_status,scope_status,source_kind,source_info,asset_refs,render_mode,source_item_key,parent_source_item_key,content_fingerprint,question_revision_token")
        .in("id", questionIds)
        .eq("grade_band", delivery.sourceGrade)
        .eq("review_status", "approved")
        .eq("scope_status", "IN")
      ...
      if (!expectedIds) questionQuery = questionQuery.in("skill_id", planSkillIds);
      if (plan.mode === "REVIEW" && !delivery.managed) questionQuery = questionQuery.not("concept_key", "is", null);
      ...
      if (!expectedIds && maxQuestionLevel !== null) questionQuery = questionQuery.lte("level", maxQuestionLevel);
      ...
      const securedQuestionIds = (questionResult.data || [])
        .filter((question) => plan.mode === "REVIEW" && (delivery.managed || isLicensedHighSchoolQuestion(question)))
        .map((question) => String(question.id));
      const lockedAnswerByQuestionId = new Map<string, Record<string, unknown>>();
      if (securedQuestionIds.length) {
        const lockedRows = await answerLocks(targetId, String(plan.id), (planAttemptsResult.data || []).length);
      ...
      if (plan.mode === "REVIEW") {
        ...
        const expectedPayload = expectedSubmission ?? await startPlanPayload(targetId, String(plan.id), { studentOpen: true, includeAnswerLocks: true });
      ...
      for (const submitted of submittedAnswers) {
        ...
        canonicalAnswers.push({
          ...
          question_snapshot: {
            version: 3,
            optionPractice: expectedSubmission?.questions.find((item) => item.id === question.id)?.optionPractice ?? null,
            source: "submission",
            ...
            renderMode: ["native", "image_assist", "image_primary"].includes(String(question.render_mode))
             ...
             parentSourceItemKey: question.parent_source_item_key ? String(question.parent_source_item_key) : null,
      ...
      const currentStatesResult = await supabase
        ...
        .select("skill_id,verified_level,candidate_level,stability,consecutive_errors,review_interval_index")
        .eq("student_id", targetId)
      ...
      for (const skillId of canonicalSkillIds) {
        const current = currentStateBySkill.get(skillId);
        const evidence = skillMasteryEvidence(canonicalAnswers, expectedSubmission?.optionPractice ?? [], skillId);
        const confirmed = evidence.confirmedLevel > 0;
        const stable = evidence.unresolvedErrors === 0 && (confirmed || current?.stability === "verified");
        const errors = evidence.unresolvedErrors ? Number(current?.consecutive_errors || 0) + evidence.unresolvedErrors : confirmed ? 0 : Number(current?.consecutive_errors || 0);
        computedStateBySkill.set(skillId, {
          student_id: targetId,
          skill_id: skillId,
          verified_level: Math.max(Number(current?.verified_level || 0), evidence.confirmedLevel),
          candidate_level: evidence.candidateLevel || current?.candidate_level || null,
          stability: stable ? "verified" : "learning",
          consecutive_errors: errors,
          next_review_at: new Date(completedAt.getTime() + (stable ? 3 : 1) * 86400000).toISOString(),
          review_interval_index: stable ? Math.min(4, Number(current?.review_interval_index || 0) + (confirmed ? 1 : 0)) : 0,
          last_reviewed_at: completedAtIso,
          teacher_intervention: evidence.unresolvedErrors > 0 && errors >= 3,
